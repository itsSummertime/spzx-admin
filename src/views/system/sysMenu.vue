...
import { FindTreeList, AddMenu, UpdateMenu, DeleteMenu } from '@/api/sysMenu'
...
const updateData = async () => {
  await UpdateMenu(sysMenu.value)
...
const fetchData = async () => {
  const { code, data } = await FindTreeList()
  if(code === 200){
    list.value = data
  }
}
