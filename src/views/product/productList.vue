...
import { FindAllBrand, FindBrandByCategoryId } from '@/api/brand.js'
import { FindNextList } from '@/api/category.js'
import {
  FindProductListByPage,
  AddProduct,
  UpdateProduct,
  FindProductById,
  DeleteProduct,
...
const props = {
  ...
  async lazyLoad(node, resolve) {
    ...
    const { data } = await FindNextList(node.value)
...
const fetchData = async () => {
  ...
  // 请求后端接口
  const { data } = await FindProductListByPage(
    pageNum.value,
    pageSize.value,
    queryDto.value
  )
